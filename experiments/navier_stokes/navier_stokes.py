...
def parse_args():
    ...
    parser.add_argument("--loss-weights", nargs="+", type=float, default=[1, 1, 1, 100, 100, 100, 100, 100])
...
def test_nn_loss(times=None, test_models=None):
    ...
    for time in times:
        ...
        for legend, test_model in test_models.items():
            ...
            contour(gs[result_count, 0], x, y, loss.reshape(num_test_samples, num_test_samples),
                    "loss-" + legend.split("_")[0], None, None, 20, resampled_points)
            result_count += 1
        plt.savefig(os.path.join(save_dir, "Re={}_t={}_loss.png".format(Re, time)))
        plt.savefig(os.path.join(save_dir, "Re={}_t={}_loss.pdf".format(Re, time)))
        plt.close()
...
boundary_condition_p = dde.icbc.NeumannBC(spatio_temporal_domain,
                                          p_func, on_boundary, component=2)
